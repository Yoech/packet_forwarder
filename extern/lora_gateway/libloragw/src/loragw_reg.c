...
int
reg_w_align32(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, struct lgw_reg_s r, int32_t reg_value) {
    ...
    if ((r.leng == 8) && (r.offs == 0)) {
        ...
        spi_stat += lgw_spi_wb(spi_target, spi_mux_mode, spi_mux_target, r.addr, buf,
...
int
reg_r_align32(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, struct lgw_reg_s r, int32_t *reg_value) {
    ...
    if ((r.offs + r.leng) <= 8) {
        ...
        if (r.sign == true) {
            ...
            *reg_value = (int32_t) u
                    >> (32 - r.leng); /* right-align the data with sign extension (ARITHMETIC right shift) */
        } else {
...
int lgw_connect(const char *spidev_path, bool spi_only, uint32_t tx_notch_freq) {
    int spi_stat = LGW_SPI_SUCCESS;
    uint8_t u = 0;
    int x;
    ...
    if (spi_only == false) {
        ...
        if (u != loregs[LGW_VERSION].dflt) {
            DEBUG_PRINTF("ERROR: NOT EXPECTED CHIP VERSION (%u/%u)\n", u, loregs[LGW_VERSION].dflt);
            return LGW_REG_ERROR;
        }
