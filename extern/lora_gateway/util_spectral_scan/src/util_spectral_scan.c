...
int main(int argc, char **argv) {
        ...
        switch (i) {
            case 'h':
                ...
                printf("                               start>%3.3f step>%1.3f stop<%3.3f\n", MIN_FREQ / 1e6,
                       MIN_STEP_FREQ / 1e6, MAX_FREQ / 1e6);
            ...
            case 'f': /* -f <float>:<float>:<float>  Frequency vector to scan in MHz, start:step:stop */
                ...
                if ((j != 3) || (arg_lf[0] < MIN_FREQ / 1e6) || (arg_lf[0] > MAX_FREQ / 1e6) ||
                    (arg_lf[1] < MIN_STEP_FREQ / 1e6) || (arg_lf[2] < MIN_FREQ / 1e6) || (arg_lf[2] > MAX_FREQ / 1e6)) {
                    printf("ERROR: argument parsing of -f argument. -h for help.\n");
                    return EXIT_FAILURE;
                } else {
                    start_freq = (uint32_t) ((arg_lf[0] * 1e6) +
                                             0.5); /* .5 Hz offset to get rounding instead of truncating */
                    step_freq = (uint32_t) ((arg_lf[1] * 1e6) +
                                            0.5); /* .5 Hz offset to get rounding instead of truncating */
                    stop_freq = (uint32_t) ((arg_lf[2] * 1e6) +
                                            0.5); /* .5 Hz offset to get rounding instead of truncating */
                }
                break;
    ...
    x = lgw_connect("/dev/spidev0.0", true, 0); /* SPI only, no FPGA reset/configure (for now) */
    if (x != 0) {
    ...
    if (TAKE_N_BITS_FROM((uint8_t) reg_val, FPGA_FEATURE_LBT, 1) == true) {
        ...
        if (stop_freq > (init_freq + 255 * LBT_MIN_STEP_FREQ)) {
            printf("ERROR: stop frequency %d is not supported, should be <%d\n", stop_freq,
                   init_freq + 255 * LBT_MIN_STEP_FREQ);
            return EXIT_FAILURE;
        }
        ...
        x = lgw_connect("/dev/spidev0.0", false, LGW_DEFAULT_NOTCH_FREQ); /* FPGA reset/configure */
        if (x != 0) {
            printf("ERROR: Failed to connect to FPGA\n");
            return EXIT_FAILURE;
        }
